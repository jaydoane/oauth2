...
-define(SECTION, "oauth2").

-define(ALLOWED_ALGOS, [<<"RS256">>]).
-define(DEFAULT_AUTH_SERVER_PEM,
...
config(auth_server_pem) ->
    config:get(?SECTION, "auth_server_pem", ?DEFAULT_AUTH_SERVER_PEM).
...
verify() ->
    ...
    PEM = list_to_binary(config(auth_server_pem)),
    verify(id_token(), jose_jwk:from_pem(PEM)).

verify(Token) ->
    PEM = list_to_binary(config(auth_server_pem)),
    verify(Token, jose_jwk:from_pem(PEM)).
